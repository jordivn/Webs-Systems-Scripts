...
# THIS VERSION IS FOR RSPAMD used in directadmin (Socket!!!)
...
then
echo -e `basename ${DOMAINS}` "\t" `basename ${MAILBOX}` "\tHAM\t" `basename ${HAMMAILBOXFOLDER}` "\t" `rspamc -h /var/run/rspamd/rspamd_controller.sock learn_ham  "${HAMMAILBOXFOLDER}/cur"` >> $LOGPATH$USER.log
fi
done
...
then
echo -e `basename ${DOMAINS}` "\t" `basename ${MAILBOX}` "\tSPAM\t" `basename ${SPAMMAILBOXFOLDER}` "\t" `rspamc -h /var/run/rspamd/rspamd_controller.sock learn_spam "${SPAMMAILBOXFOLDER}/cur"` >> $LOGPATH$USER.log
fi
done
fi
done
...
echo "Restarting RSPAMD" >> /var/log/sa-learn/overall.log
